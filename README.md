...
Once we have imported the above data into the software we can start exploring the metagenomics contigs. It is possible to inspect the clustering of the contigs further up to eight dimensions of the PCA analysis while one can also investigate the features of contigs in the context of sample coverages. The user can choose to switch the view of the PCA panel to a ‘coverage’ mode and inspect the coverage information for the contigs across the samples of the dataset. The size of the contigs in the PCA space shows the coverages of the contigs and serves as a visual cue for the species that are abundant in a given sample with a slider bar giving a means to shift between samples. Additional metadata associated with the samples can be imported from a file, in order to explore different treatment groups (e.g., samples for CD individuals vs. healthy individuals). Set of buttons provides additional functionalities by showing/hiding the legends with the information for the dataset clusters (cluster number, taxonomy etc.) and adjusting the font size for better representation. This is demonstrated in the video right below:
