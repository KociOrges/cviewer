<img width="200" height="200" alt="Screenshot 2020-08-10 at 11 30 22" src="https://github.com/KociOrges/cviewer/blob/master/documentation/cviewer-logo-minimal.png">
...
To obtain the COGs, we will first use the [RPS-BLAST](https://warwick.ac.uk/fac/sci/moac/people/students/peter_cock/python/rpsblast/) to find COG annotation for the protein sequences using the script RPSBLAST.sh that is provided with the CONCOCT software. A complete example for doing this is given [here](https://concoct.readthedocs.io/en/0.3.3/complete_example.html). The blast output will be a _PROKKA_XXXXXXX.out_ file which you can then give as an input to the script _ContigsCOGsDB.java_ to generate COG assignments for the protein sequences. Assuming that you are in the parent directory (CViewer), you can do this as follows:
```
...
Community phylogenetic alpha diversity analysis is also possible with ```CViewer``` software to explore environmental pressures and give an indication of stochasticity. We have considered two  metrics which, have been suggested by Webb[[4]](#4) and look for phylogenetic clustering or overdispersion, followed by employing a randomization procedure called statistical effect size (SES) to calculate the _Net Relatedness Index_ (NRI) and _Nearest Taxon Index_ (NTI). 

**Step 1: Extracting a gene from the metagenomics contigs**

Before extracting the gene data, we need to **ensure that the output from the CONCOCT (PCA file, coverage, and clustering files; see Section: Visualising the metagenomics contigs) and PROKKA (GenBank file) pipelines are imported into the software**. Then, to perform phylogenetic analysis with CViewer, one first needs to generate a phylogenetic tree. To do this, the system provides a set of essential genes[[5]](#5) that can be extracted from the contigs for creating a phylogenetic tree with support for manual selection as well. After the gene has been specified, the gene sequences are extracted for each cluster of the metagenomics dataset and are saved in a ```.fasta``` file in the *Output/* folder, along with the average coverages of the clusters across all the samples. 
...
Once we have extracted the sequences in a ```.fasta``` file, we can use third-party tools to generate the phylogenetic tree, such as [MUSCLE](http://www.drive5.com/muscle/) for performing the alignment and [FastTree](http://www.microbesonline.org/fasttree/) for producing the tree. Once this is accomplished, the obtained tree and the table describing the abundances across the dataset clusters (generated from the previous step) are uploaded to the software, to calculate the ***```NRI```*** and ***```NTI```*** indices. 
...
[![Picture 1](https://github.com/KociOrges/cviewer/blob/master/documentation/friedman-cviewer-demo-thumbnail.png)](https://youtu.be/WgGwZdBQEp0?si=-NUJLs7ELeuKzhnt)
